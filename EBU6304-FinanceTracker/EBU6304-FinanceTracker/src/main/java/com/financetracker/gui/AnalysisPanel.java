...
public class AnalysisPanel extends JPanel {
    ...
    private void addChatBubble(String message, boolean isUser) {
        int maxWidth = chatScrollPane != null ? chatScrollPane.getViewport().getWidth() : 350;
        int bubbleWidth = (maxWidth - 60) / 2;

        JPanel rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        rowPanel.setOpaque(false);

        JTextArea userBubble = new JTextArea();
        userBubble.setLineWrap(true);
        userBubble.setWrapStyleWord(true);
        userBubble.setEditable(false);
        userBubble.setFont(new Font("微软雅黑", Font.PLAIN, 15));
        userBubble.setMaximumSize(new Dimension(bubbleWidth, Integer.MAX_VALUE));

        JTextArea aiBubble = new JTextArea();
        aiBubble.setLineWrap(true);
        aiBubble.setWrapStyleWord(true);
        aiBubble.setEditable(false);
        aiBubble.setFont(new Font("微软雅黑", Font.PLAIN, 15));
        aiBubble.setMaximumSize(new Dimension(bubbleWidth, Integer.MAX_VALUE));

        if (isUser) {
            userBubble.setText(message);
            userBubble.setBackground(new Color(194, 241, 255));
            userBubble.setBorder(BorderFactory.createEmptyBorder(8, 12, 8, 12));
            aiBubble.setBackground(new Color(0,0,0,0)); // 透明
            aiBubble.setBorder(null);
            rowPanel.add(Box.createHorizontalGlue());
            rowPanel.add(userBubble);
            rowPanel.add(Box.createHorizontalStrut(24)); // 3个字符宽度
            rowPanel.add(aiBubble);
        } else {
            aiBubble.setText(message);
            aiBubble.setBackground(new Color(230, 230, 230));
            aiBubble.setBorder(BorderFactory.createEmptyBorder(8, 12, 8, 12));
            userBubble.setBackground(new Color(0,0,0,0)); // 透明
            userBubble.setBorder(null);
            rowPanel.add(userBubble);
            rowPanel.add(Box.createHorizontalStrut(24)); // 3个字符宽度
            rowPanel.add(aiBubble);
            rowPanel.add(Box.createHorizontalGlue());
        ...
        JPanel timePanel = new JPanel(new FlowLayout(isUser ? FlowLayout.RIGHT : FlowLayout.LEFT, 0, 0));
        timePanel.setOpaque(false);
        timePanel.add(timeLabel);

        chatPanel.add(rowPanel);
        chatPanel.add(timePanel);
